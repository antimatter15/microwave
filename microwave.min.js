...
function animated_scroll(a,c){var b,d=+new Date+500,f=a.scrollTop;(b=function(){var e=1-(d-new Date)/500;if(e<1){c=0;a.scrollTop=(f-c)*e+f;setTimeout(b,0)}})()}function scroll_wavepanel(a){if(opt.multipane)if(opt.touchscroll)touchscroll0.scrollTo(0,a);else document.getElementById("wave_container_parent").scrollTop=a;else scrollTo(0,a)}
function scroll_searchpanel(a){if(opt.multipane)if(opt.touchscroll)touchscroll1.scrollTo(0,a);else document.getElementById("search_parent_container").scrollTop=a;else scrollTo(0,a)}function blip_next(a){try{for(var c=[].indexOf?chronological_blips.indexOf(a):function(d,f,e){for(e=f.length;e--&&f[e]!=d;);return e}(a,chronological_blips);c&&blip_scroll(--c)==false;);}catch(b){}}function hide_float(){document.getElementById("floating_menu").className=""}
...
a));else i.type!="LINE"&&console.log("unknown element type",i.type,i.properties);k=document.createElement(i.properties.lineType||"p");if(i.properties.indent)k.style.marginLeft=i.properties.indent*20+"px";if(i.properties.alignment)k.style.textAlign={l:"left",c:"center",r:"right"}[i.properties.alignment];i.properties.direction&&k.setAttribute("dir",{l:"ltr",r:"rtl"}[i.properties.alignment]);j.appendChild(k)}if(b[e]){o=b[e];for(var r=o.length;r--;){g=a.annotations[o[r]];h[g.name]||(h[g.name]=[]);h[g.name].push(g.value);
if(g.name.indexOf("user/e/")==0){i=g.name.substr(7);if(f[i]){var q=document.createElement("span");q.className="cursor";q.innerHTML=g.value.replace(/@.+/,"");q.style.backgroundColor=f[i];l.appendChild(q)}}}}if(d[e]){o=d[e];for(r=o.length;r--;){g=a.annotations[o[r]];h[g.name].shift();h[g.name].length==0&&delete h[g.name]}}l=h["link/auto"]||h["link/manual"]||h["link/wave"]?document.createElement("a"):document.createElement("span");k.appendChild(l);for(g in h){i=h[g][0];if(g.indexOf("style/")==0)l.style[g.substr(6)]=
i;else if(g=="conv/title")l.style.fontWeight="bold";else if(g!="spell")if(g!="lang")if(g=="link/manual"||g=="link/auto")if(/^waveid:\/\//.test(i)){l.href="#wave:"+i.substr(9);l.setAttribute("onclick","ch(this)")}else if(/wave:(.+?)(\,$)/.test(i)){l.href="#wave:"+i.match(/wave:(.+?)(\,$)/)[1];l.setAttribute("onclick","ch(this)")}else{l.href=i;l.target="_blank"}else if(g=="link/wave"){l.href="#wave:"+i;l.setAttribute("onclick","ch(this)")}else g.indexOf("user/e")!=0&&g.indexOf("user/d")!=0&&g.indexOf("user/r")!=
0&&console.log("unrecognized annotation",g,i)}}if(c.charAt(e)!="\n")p+=c.charAt(e)}p&&l.appendChild(document.createTextNode(p));return j}var queue=[],callbacks={},id_count=0;function queueOp(a,c,b){var d=(id_count++).toString();if(b)callbacks[d]=b;queue.push({id:d,method:a,params:c});return d}
...
function extend_search(a,c){searchLastIndex=a;search_outdated=false;callbacks[wave.robot.search(current_search,a||0,42)]=function(b){c&&c();h=b;console.log(h);b="";var d,f=h.data.searchResults.digests;if(opt.prefetch){var e=0;setTimeout(function(){var j=f[e++];if(j){var k=j.waveId.replace(/[\/!].+/,"!conv+root");callbacks[wave.robot.fetchWave(j.waveId,k)]=function(l){if(l)prefetched_waves[j.waveId]=l;return true};runQueue();setTimeout(arguments.callee,500)}},1E3)}for(var g=0;g<f.length;g++){d=f[g];
...
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,e,g,h={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},i;if(typeof JSON.stringify!=="function")JSON.stringify=function(j,k,l){var p;g=e="";if(typeof l==="number")for(p=0;p<l;p+=1)g+=" ";else if(typeof l==="string")g=l;if((i=k)&&typeof k!=="function"&&(typeof k!=="object"||typeof k.length!=="number"))throw Error("JSON.stringify");return b("",

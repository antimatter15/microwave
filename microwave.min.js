...
if(opt.keyboard)document.body.onkeydown=function(a){if(a&&a.target)if(a.target.tagName=="BODY")if(a.shiftKey&&a.keyCode==32||!a.shiftKey&&!a.ctrlKey&&a.keyCode==75){blip_prev(lastscrolled);a.preventDefault()}else if(a.keyCode==32||!a.shiftKey&&!a.ctrlKey&&a.keyCode==74){blip_next(lastscrolled);a.preventDefault()}};
opt.swipe&&function(){var a=0,c=0,b=0,d=0,f,e=0,g=function(j){if(j.touches.length==1){b=j.touches[0].pageX;d=j.touches[0].pageY;f=j.touches[0].target;e=+new Date}},i=function(j){if(j.touches.length==1){a=j.touches[0].pageX;c=j.touches[0].pageY}},h=function(){var j=+new Date-e,l=a-b,k=c-d,m=f;if(m){for(;!m.blipId&&m!=document.body;)m=m.parentNode;if(m.blipId)if(Math.abs(k)<30&&j<500)if(Math.abs(l)>60)l>0?blip_next(m.blipId):blip_prev(m.blipId)}};if(mobilewebkit){document.body.addEventListener("touchstart",
g,true);document.body.addEventListener("touchmove",i,true);document.body.addEventListener("touchend",h,true)}else{document.body.addEventListener("mousedown",function(j){g({touches:[j]})},true);document.body.addEventListener("mousemove",function(j){i({touches:[j]})},true);document.body.addEventListener("mouseup",function(j){h({touches:[j]})},true)}}();
...
f.innerHTML="<div style='float:right;color:#555'>"+format_time(b.lastModifiedTime).toString()+e;f.appendChild(userList(b.contributors));try{d.addEventListener("dragenter",function(i){i.stopPropagation();i.preventDefault()},false);d.addEventListener("dragover",function(i){i.stopPropagation();i.preventDefault();loading("Drop file to upload! (Plz no big files)")},false);d.addEventListener("dragend",function(i){i.stopPropagation();i.preventDefault();loading()},false);d.addEventListener("drop",function(i){d.style.backgroundColor=
"";i.stopPropagation();i.preventDefault();i=i.dataTransfer.files;for(var h=0;h<i.length;h++){var j=i[h],l=new FileReader;l.onload=function(k){k=k.target.result;if(/^data:.+;base64,/.test(k)){k=k.substr(k.indexOf(",")+1);wave.blip.upload_attachment(k,j.name,a,b.waveId,b.waveletId);loading("Uploading file "+j.name);runQueue()}else{console.log(k.substr(0,50));alert("data URL is not base64!")}};l.readAsDataURL(j)}},false)}catch(g){console.log(g)}d.onclick=doNothing;f.onclick=doNothing;d.insertBefore(f,
d.firstChild);c.appendChild(d);a==chronological_blips[scrollto_position]&&setTimeout(function(){blip_scroll(scrollto_position)},500);return d}}
...
function renderGadget(a,c){var b={},d=[],f;for(f in a.properties){if(f!="url"&&f!="author")b[f]=a.properties[f];d.push(f)}var e=document.createElement("div");e.style.margin="10px";var g=a.properties.url;e.innerHTML="<b>gadget</b> "+g+" <br>";if(window.opt.gadgets){load_native_gadget(b,a,c,e);return e}if(g=="http://wave-api.appspot.com/public/gadgets/areyouin/gadget.xml"){var i={y:[],n:[],m:[]};for(f in b)/:answer$/.test(f)&&i[b[f]].push(f.substr(0,f.length-7));for(var h in i){e.innerHTML+="<br><span style='color:red;font-weight:bold'>"+
...
return f}}function error(a){var c=getEl("error");c.style.display="";getEl("errortext").innerHTML+="<div><b>Error:</b> "+a+"</div>";c.onclick=function(){c.style.display="none";getEl("errortext").innerHTML=""}}var lastscrolled="",gstates={};
...
"'>Download</a>";j.appendChild(l)}else if(h.type=="GADGET")j.appendChild(renderGadget(h,a));else h.type!="LINE"&&console.log("unknown element type",h.type,h.properties);l=document.createElement(h.properties.lineType||"p");if(h.properties.indent)l.style.marginLeft=h.properties.indent*20+"px";if(h.properties.alignment)l.style.textAlign={l:"left",c:"center",r:"right"}[h.properties.alignment];h.properties.direction&&l.setAttribute("dir",{l:"ltr",r:"rtl"}[h.properties.alignment]);j.appendChild(l)}if(b[e])for(var n=
b[e],o=n.length;o--;){g=a.annotations[n[o]];i[g.name]||(i[g.name]=[]);i[g.name].push(g.value);if(g.name.indexOf("user/e/")==0){h=g.name.substr(7);if(f[h]){var p=document.createElement("span");p.className="cursor";p.innerHTML=g.value.replace(/@.+/,"");p.style.backgroundColor=f[h];k.appendChild(p)}}}if(d[e]){n=d[e];for(o=n.length;o--;){g=a.annotations[n[o]];i[g.name].shift();i[g.name].length==0&&delete i[g.name]}}k=i["link/auto"]||i["link/manual"]||i["link/wave"]?document.createElement("a"):document.createElement("span");
...
function runQueue(){if(queue.length==0)return false;for(var a=[],c=0;c<queue.length;c++)a.push(queue[c].id);doXHR(JSON.stringify(queue),function(b){if(b.status==200){var d;try{d=JSON.parse(b.responseText)}catch(f){if(b.responseText.indexOf("Error 401")!=-1)return logoff_ui();for(var e=0;e<a.length;e++){var g=null,i=a[e];if(callbacks[i]){try{g=callbacks[i]()}catch(h){}delete callbacks[i]}if(!g){console.log("Server Error: Could not parse as JSON",b.responseText);error("JSON not parseable.")}}}if(d){console.log(d);
for(e=0;e<d.length;e++){i=d[e].id;g=null;if(callbacks[i]){g=callbacks[i](d[e]);delete callbacks[i]}if(d[e].error&&!g){if(d[e].error.code==401)return logoff_ui();console.log("Error "+d[e].error.code+": "+d[e].error.message);error(d[e].error.message)}}}}else for(e=0;e<a.length;e++){g=null;i=a[e];if(callbacks[i]){try{g=callbacks[i]()}catch(j){}delete callbacks[i]}if(!g){console.log("Server Error: Error not caught.",b.status);error("No data was returned in the server response.")}}});queue=[]}
...
"</span>":"<span class='read_msg'>"+d.blipCount+" msgs</span>",j=format_time(d.lastModified),l=opt.offline?' <span class="cachestate">cache</span>':"";h='<div id="'+d.waveId+'" class="search '+resultClass(d.waveId)+'"><div class="date">'+j+"<br>"+h+l+'</div><span class="title_'+(d.unreadCount>0?"unread":"read")+'">'+d.title+"</span> <span class='snippet'>"+d.snippet+"</span> </div>";f+='<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'>'+h+"</a>"}}search_container.innerHTML+=f;
...
db.transaction(function(g){g.executeSql("SELECT * FROM inbox WHERE waveid = ?",[a],function(i,h){var j=JSON.parse(h.rows.item(0).data);e(j)})})}else if(opt.prefetch&&prefetched_waves[a])e(JSON.parse(JSON.stringify(prefetched_waves[a])));else callbacks[wave.robot.fetchWave(a,c)]=e}
if(!this.JSON)this.JSON={stringify:function(a){var c=JSON.stringify;if(typeof a=="boolean"||typeof a=="number"||a===null||typeof a=="undefined")return a+"";else if(typeof a=="string")return'"'+a.replace(/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,function(f){return"\\u"+("0000"+f.charCodeAt(0).toString(16)).slice(-4)})+'"';else if(a.length){for(var b=0;b<a.length;b++)a[b]=c(a[b]);return"["+a.join(",")+"]"}else{b=[];for(var d in a)b.push(c(d)+

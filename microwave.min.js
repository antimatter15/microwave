...
function create_contextmenu(a){function c(){d.style.display="none";d.parentNode.removeChild(d)}if(!onLine())return document.createElement("div");try{ctx_menu.style.display="none";ctx_menu.parentNode.removeChild(ctx_menu)}catch(b){}var d=document.createElement("div");current_blip=a;d.style.zIndex=32;var f={Reply:function(){current_blip.dom.parentNode.insertBefore(create_context_box(),current_blip.dom.nextSibling);context_box.className=a.childBlipIds.length>0?"thread":"";context_box.style.display="";
reply_text.focus();c()},Delete:function(){confirm("Are you sure you want to delete the blip?")&&setTimeout(function(){wave.blip["delete"](current_blip.blipId,current_blip.waveId,current_blip.waveletId);loadWave(current_blip.waveId);auto_reload=true;runQueue();c()},100);c()},Edit:function(){current_blip.dom.parentNode.insertBefore(create_edit_box(),current_blip.dom.nextSibling);var h=0;try{if(current_blip.annotations[0].name=="conv/title")h=current_blip.annotations[0].range.end}catch(i){}edit_text.value=
current_blip.content.substr(h+1);edit_text.focus();c()}};if(a.blipId==msg.data.waveletData.rootBlipId)f["Change Title"]=function(){var h=prompt("Enter new wavelet title",msg.data.waveletData.title);if(h){wave.wavelet.setTitle(h,a.waveId,a.waveletId);loadWave(a.waveId,a.waveletId);auto_reload=true;runQueue()}};for(var e in f){var g=document.createElement("a");g.href="javascript:void(0)";g.onclick=f[e];g.innerHTML=e;d.appendChild(g);d.appendChild(document.createTextNode(" / "))}g=document.createElement("a");
g.href="javascript:void(0);";g.style.color="red";g.innerHTML="Close";g.onclick=function(){c()};d.appendChild(g);d.className="contextmenu";return ctx_menu=d}
...
setTimeout(function(){var a=0;try{if(current_blip.annotations[0].name=="conv/title")a=current_blip.annotations[0].range.end}catch(c){}var b=diff(current_blip.content.substr(a),"\n"+edit_text.value);console.log(b);console.log(current_blip.content,"\n"+edit_text.value);wave.blip.replace_range(b[2],a+b[0],a+b[1],current_blip.blipId,current_blip.waveId,current_blip.waveletId);loadWave(current_blip.waveId);auto_reload=true;runQueue()},100)};edit_box.style.marginTop="10px";edit_box.appendChild(edit_text);
edit_box.appendChild(submit_edit);edit_box.appendChild(cancel_edit);edit_text.style.height=Math.max(current_blip.dom.offsetHeight,100)+"px";edit_text.className="edit_box";return edit_box}var gstates={},REMOTE_RPC_RELAY_URL="http://www.gmodules.com/gadgets/files/container/rpc_relay.html",DEFAULT_GADGET_MODE={"${playback}":"0","${edit}":"1"};function registerRpc(a,c){gadgets.rpc.register(a,function(){c(this.s,this.f,this.a)})}
...
username="wheisenberg@googlewave.com";function getUsername(){callbacks[wave.robot.fetchWave("googlewave.com!w+bWEBb5mBA","googlewave.com!conv+poop")]=function(a){username=a.error.message.match(/internalError: (.+@.+) is not a participant/)[1];return true}}opt.x.use_protocol_21="Use old 0.21 version of wave protocol";
...
function thread_render(a,c){var b=msg.data.blips[a];if(!b.dom){var d=blip_render(a,c);b=b.replyThreadIds;var f=b.length;if(f!=0){var e=document.createElement("div");e.className="thread";c.appendChild(e);for(t=0;t<f;t++)for(var g=msg.data.threads[b[t]].blipIds,h=g.length,i=0;i<h;i++)thread_render(g[i],e)}return d}}
...
[e]}h={};var j=document.createElement("div"),k=null,l=null,o="";for(e=0;e<c.length;e++){if(b[e]||d[e]||a.elements[e]){o&&l.appendChild(document.createTextNode(o));o="";if(a.elements[e]){i=a.elements[e];if(i.type=="INLINE_BLIP")j.appendChild(inline_blip_render(i.properties.id));else if(i.type=="IMAGE"){k=document.createElement("div");k.innerHTML="<b>"+(i.properties.attachmentId||"")+"</b> "+(i.properties.caption||"")+"<br>";if(i.properties.url){var n=document.createElement("img");n.src=i.properties.url;
if(small_screen){n.style.width="100%";n.onclick=function(){n.style.width=n.style.width.indexOf("%")==-1?"100%":""}}k.appendChild(n)}j.appendChild(k)}else if(i.type=="INSTALLER"){k=document.createElement("div");k.style.border="3px dotted orange";k.style.margin="10px";k.innerHTML="&lt;<b>Extension Installer</b> "+i.properties.manifest+"&gt;";j.appendChild(k)}else if(i.type=="ATTACHMENT"){k=document.createElement("div");k.style.margin="10px";k.innerHTML="<b>"+i.properties.mimeType+"</b> "+i.properties.caption+
"<br>";if(i.properties.mimeType.indexOf("image/")==0){n=document.createElement("img");n.src=i.properties.attachmentUrl;if(small_screen){n.style.width="100%";n.onclick=function(){n.style.width=n.style.width.indexOf("%")==-1?"100%":""}}k.appendChild(n)}else k.innerHTML+="<a href='"+i.properties.attachmentUrl+"'>Download</a>";j.appendChild(k)}else if(i.type=="GADGET")j.appendChild(renderGadget(i,a));else i.type!="LINE"&&console.log("unknown element type",i.type,i.properties);k=document.createElement(i.properties.lineType||
"p");if(i.properties.indent)k.style.marginLeft=i.properties.indent*20+"px";if(i.properties.alignment)k.style.textAlign={l:"left",c:"center",r:"right"}[i.properties.alignment];i.properties.direction&&k.setAttribute("dir",{l:"ltr",r:"rtl"}[i.properties.alignment]);j.appendChild(k)}if(b[e])for(var r=b[e],q=r.length;q--;){g=a.annotations[r[q]];h[g.name]||(h[g.name]=[]);h[g.name].push(g.value);if(g.name.indexOf("user/e/")==0){i=g.name.substr(7);if(f[i]){var p=document.createElement("span");p.className=
"cursor";p.innerHTML=g.value.replace(/@.+/,"");p.style.backgroundColor=f[i];l.appendChild(p)}}}if(d[e]){r=d[e];for(q=r.length;q--;){g=a.annotations[r[q]];h[g.name].shift();h[g.name].length==0&&delete h[g.name]}}l=h["link/auto"]||h["link/manual"]||h["link/wave"]?document.createElement("a"):document.createElement("span");k.appendChild(l);for(g in h){i=h[g][0];if(g.indexOf("style/")==0)l.style[g.substr(6)]=i;else if(g=="conv/title")l.style.fontWeight="bold";else if(g!="spell")if(g!="lang")if(g=="link/manual"||
g=="link/auto"){l.href=i;l.target="_blank"}else if(g=="link/wave"){l.href="#wave:"+i;l.setAttribute("onclick","ch(this)")}else g.indexOf("user/e")!=0&&g.indexOf("user/d")!=0&&g.indexOf("user/r")!=0&&console.log("unrecognized annotation",g,i)}}if(c.charAt(e)!="\n")o+=c.charAt(e)}o&&l.appendChild(document.createTextNode(o));return j}var queue=[],callbacks={},id_count=0;function queueOp(a,c,b){var d=(id_count++).toString();if(b)callbacks[d]=b;queue.push({id:d,method:a,params:c});return d}
...
-1?"<div class='footer'><b>Yay</b>, no unread items!</div>":"<div class='footer'>Your search query has <b>zero results</b>. Try broaden your search?</div>":"<div class='footer'><b>Hooray!</b> You've reached the end of the universe!</div>":'<div class="footer" onclick="auto_extend(this);"><b>Extend</b> Search (Page '+(a/42+1)+")</div>"}}var chronological_blips=[];
...
"";window.onscroll();document.getElementById("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var e=Object.keys(msg.data.blips);else{e=[];for(var g in msg.data.blips)e.push(g)}chronological_blips=e.sort(function(h,i){return msg.data.blips[i].lastModifiedTime-msg.data.blips[h].lastModifiedTime})}scrollto_position=-1;if(!auto_reload)if(!opt.no_scrollhistory){scrollTo(0,0);if(!opt.no_autoscroll)if(unread_blips[a])scrollto_position=unread_blips[a]}auto_reload=false;if(!opt.multipane)document.getElementById("suggest").style.display=
"none";wave_container.innerHTML="";if(opt.owner_utils&&msg.data.waveletData.participants[0]==username)wave_container.innerHTML+='<div><button onclick="set_user_mode()">Everyone Read Only</button><button onclick="set_user_mode(\'FULL\')">Everyone Full Access</button></div>';e=document.createElement("div");e.className="wavelet";e.appendChild(userList(f.data.waveletData.participants));g=document.createElement("a");g.innerHTML=" Add Participant";g.className="addparticipant";g.href="javascript:void(0)";
g.onclick=function(){var h=prompt("Enter Participant ID to Add");if(h){if(h.indexOf("@")==-1)h+="@googlewave.com";wave.wavelet.participant.add(h,a,c);loadWave(a);auto_reload=true;runQueue()}};e.appendChild(g);wave_container.appendChild(e);current_wave=a=a.replace(/\s/,"");current_wavelet=c;e=document.createElement("div");wave_container.appendChild(e);if(document.getElementById("chronos").checked)chronological_blip_render(e);else opt.recursive_renderer||!msg.data.waveletData.rootThread?recursive_blip_render(msg.data.waveletData.rootBlipId,
...
f.className="footer";wave_container.appendChild(f)}else alert("The server sent nothing")};if(opt.prefetch&&prefetched_waves[a])d(JSON.parse(JSON.stringify(prefetched_waves[a])));else callbacks[wave.robot.fetchWave(a,c)]=d}if(!this.JSON)this.JSON={};
(function(){function a(j){return j<10?"0"+j:j}function c(j){f.lastIndex=0;return f.test(j)?'"'+j.replace(f,function(k){var l=h[k];return typeof l==="string"?l:"\\u"+("0000"+k.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+j+'"'}function b(j,k){var l,o,n,r,q=e,p,m=k[j];if(m&&typeof m==="object"&&typeof m.toJSON==="function")m=m.toJSON(j);if(typeof i==="function")m=i.call(k,j,m);switch(typeof m){case "string":return c(m);case "number":return isFinite(m)?String(m):"null";case "boolean":case "null":return String(m);
case "object":if(!m)return"null";e+=g;p=[];if(Object.prototype.toString.apply(m)==="[object Array]"){r=m.length;for(l=0;l<r;l+=1)p[l]=b(l,m)||"null";n=p.length===0?"[]":e?"[\n"+e+p.join(",\n"+e)+"\n"+q+"]":"["+p.join(",")+"]";e=q;return n}if(i&&typeof i==="object"){r=i.length;for(l=0;l<r;l+=1){o=i[l];if(typeof o==="string")if(n=b(o,m))p.push(c(o)+(e?": ":":")+n)}}else for(o in m)if(Object.hasOwnProperty.call(m,o))if(n=b(o,m))p.push(c(o)+(e?": ":":")+n);n=p.length===0?"{}":e?"{\n"+e+p.join(",\n"+e)+
"\n"+q+"}":"{"+p.join(",")+"}";e=q;return n}}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(){return isFinite(this.valueOf())?this.getUTCFullYear()+"-"+a(this.getUTCMonth()+1)+"-"+a(this.getUTCDate())+"T"+a(this.getUTCHours())+":"+a(this.getUTCMinutes())+":"+a(this.getUTCSeconds())+"Z":null};String.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(){return this.valueOf()}}var d=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,e,g,h={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},i;if(typeof JSON.stringify!=="function")JSON.stringify=function(j,k,l){var o;g=e="";if(typeof l==="number")for(o=0;o<l;o+=1)g+=" ";else if(typeof l==="string")g=l;if((i=k)&&typeof k!=="function"&&(typeof k!=="object"||typeof k.length!=="number"))throw Error("JSON.stringify");return b("",
{"":j})};if(typeof JSON.parse!=="function")JSON.parse=function(j,k){function l(n,r){var q,p,m=n[r];if(m&&typeof m==="object")for(q in m)if(Object.hasOwnProperty.call(m,q)){p=l(m,q);if(p!==undefined)m[q]=p;else delete m[q]}return k.call(n,r,m)}var o;j=String(j);d.lastIndex=0;if(d.test(j))j=j.replace(d,function(n){return"\\u"+("0000"+n.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(j.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){o=eval("("+j+")");return typeof k==="function"?l({"":o},""):o}throw new SyntaxError("JSON.parse");}})();
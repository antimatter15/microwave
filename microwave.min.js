...
var screen_size=document.documentElement.clientWidth||innerWidth,small_screen=screen_size<500,loadIds={},current_search="",searchLastIndex=0,edit_box,edit_text,search_container=document.getElementById("search_container"),wave_container=document.getElementById("wave_container"),mobilewebkit=navigator.userAgent.indexOf("WebKit")!=-1&&navigator.userAgent.indexOf("Mobile")!=-1,current_wave="",current_wavelet="",auto_reload=false,lasthash="chunkybacon",current_page=0,search_outdated=false,searchscroll=
0,scrollto_position=-1;if(mobilewebkit)document.body.className+=" mobilewebkit";if(!window.onLine)window.onLine=function(){return true};function searchStyle(a){if(cacheState[a]==2)return"fresh_cache";else if(cacheState[a]==1)return"old_cache";return""}
function onLine(){var a;a=opt.force_offline?false:navigator.onLine===undefined?true:navigator.onLine;if(a==false){var c=(+new Date-parseInt(localStorage.getItem("cache_last_updated")))/1E3;if(!isNaN(c)){var b="";b=c<90?"a minute":c<3600?Math.ceil(c/60)+" minutes":c<86400?Math.ceil(c/60/60)+" hours":Math.ceil(c/60/60/24)+" days";document.getElementById("offline_status").value="Offline (Cache "+b+" old)"}}return a}var cachequeue=[],db=null;
...
opt.swipe&&function(){var a=0,c=0,b=0,d=0,f,e=0,g=function(j){if(j.touches.length==1){b=j.touches[0].pageX;d=j.touches[0].pageY;f=j.touches[0].target;e=+new Date}},i=function(j){if(j.touches.length==1){a=j.touches[0].pageX;c=j.touches[0].pageY}},h=function(){var j=a-b,l=c-d,k=f;if(k){for(;!k.blipId&&k!=document.body;)k=k.parentNode;if(k.blipId)if(Math.abs(l)<10)if(Math.abs(j)>30)j>0?blip_prev(k.blipId):blip_next(k.blipId)}};if(mobilewebkit){document.body.addEventListener("touchstart",g,true);document.body.addEventListener("touchmove",
i,true);document.body.addEventListener("touchend",h,true)}else{document.body.addEventListener("mousedown",function(j){g({touches:[j]})},true);document.body.addEventListener("mousemove",function(j){i({touches:[j]})},true);document.body.addEventListener("mouseup",function(j){h({touches:[j]})},true)}}();
...
function animated_scroll(a,c){var b=a==window,d=b?pageYOffset:a.scrollTop;if(d!=c){var f=Math.min(1E3,2*Math.sqrt(42*Math.abs(c-d))),e,g=+new Date+f;(e=function(){var i=Math.min(1,1-(g-new Date)/f),h=(c-d)*(-Math.cos(i*Math.PI)/2+0.5)+d;if(b)scrollTo(0,h);else a.scrollTop=h;i<1&&setTimeout(e,0)})()}}
...
function flicker(a){document.getElementById("floating_menu").style.backgroundColor="#D1FCC9";a.style.color="green";a.style.fontWeight="bold";setTimeout(function(){a.style.color="";a.style.fontWeight="";document.getElementById("floating_menu").style.backgroundColor=""},500)}username="wheisenberg@googlewave.com";
function getUsername(){callbacks[wave.robot.fetchWave("googlewave.com!w+bWEBb5mBA","googlewave.com!conv+poop")]=function(a){username=a.error.message.match(/internalError: (.+@.+) is not a participant/)[1];return true}}function clean_text(a){return a.replace(/[\0-\x09\x0b-\x1f\x7f\x80-\x9f\u2028\u2029\ufff9\ufffa\ufffb\u200e\u200f\u202a-\u202e]/g,"")}
...
function renderBlip(a){for(var c=a.content+" ",b={},d={},f={},e=0;e<a.annotations.length;e++){var g=a.annotations[e];if(g.name.indexOf("user/d/")==0){var i=g.value.split(","),h=g.name.substr(7);if(new Date-parseInt(i[1])<6E4)f[h]="rgb("+Math.floor(205-Math.random()*100).toString()+","+Math.floor(205-Math.random()*100).toString()+","+Math.floor(205-Math.random()*100).toString()+")"}if(b[g.range.start])b[g.range.start].push(e);else b[g.range.start]=[e];if(d[g.range.end])d[g.range.end].push(e);else d[g.range.end]=
[e]}i={};var j=document.createElement("div"),l=null,k=null,n="";for(e=0;e<c.length;e++){if(b[e]||d[e]||a.elements[e]){n&&k.appendChild(document.createTextNode(n));n="";if(a.elements[e]){h=a.elements[e];if(h.type=="INLINE_BLIP")j.appendChild(inline_blip_render(h.properties.id));else if(h.type=="IMAGE"){l=document.createElement("div");l.innerHTML="<b>"+(h.properties.attachmentId||"")+"</b> "+(h.properties.caption||"")+"<br>";h.properties.url&&renderImage(h.properties.url,l);j.appendChild(l)}else if(h.type==
"INSTALLER"){l=document.createElement("div");l.style.border="3px dotted orange";l.style.margin="10px";l.innerHTML="&lt;<b>Extension Installer</b> "+h.properties.manifest+"&gt;";j.appendChild(l)}else if(h.type=="ATTACHMENT"){l=document.createElement("div");l.style.margin="10px";l.innerHTML="<b>"+h.properties.mimeType+"</b> "+h.properties.caption+"<br>";if(h.properties.mimeType.indexOf("image/")==0)renderImage(h.properties.attachmentUrl,l);else l.innerHTML+="<a href='"+h.properties.attachmentUrl+"'>Download</a>";
j.appendChild(l)}else if(h.type=="GADGET")j.appendChild(renderGadget(h,a));else h.type!="LINE"&&console.log("unknown element type",h.type,h.properties);l=document.createElement(h.properties.lineType||"p");if(h.properties.indent)l.style.marginLeft=h.properties.indent*20+"px";if(h.properties.alignment)l.style.textAlign={l:"left",c:"center",r:"right"}[h.properties.alignment];h.properties.direction&&l.setAttribute("dir",{l:"ltr",r:"rtl"}[h.properties.alignment]);j.appendChild(l)}if(b[e])for(var o=b[e],
r=o.length;r--;){g=a.annotations[o[r]];i[g.name]||(i[g.name]=[]);i[g.name].push(g.value);if(g.name.indexOf("user/e/")==0){h=g.name.substr(7);if(f[h]){var p=document.createElement("span");p.className="cursor";p.innerHTML=g.value.replace(/@.+/,"");p.style.backgroundColor=f[h];k.appendChild(p)}}}if(d[e]){o=d[e];for(r=o.length;r--;){g=a.annotations[o[r]];i[g.name].shift();i[g.name].length==0&&delete i[g.name]}}k=i["link/auto"]||i["link/manual"]||i["link/wave"]?document.createElement("a"):document.createElement("span");
...
"ch(this)")}else g.indexOf("user/e")!=0&&g.indexOf("user/d")!=0&&g.indexOf("user/r")!=0&&console.log("unrecognized annotation",g,h)}}if(c.charAt(e)!="\n")n+=c.charAt(e)}n&&k.appendChild(document.createTextNode(n));return j}var queue=[],callbacks={},id_count=0;function queueOp(a,c,b){var d=(id_count++).toString();if(b)callbacks[d]=b;queue.push({id:d,method:a,params:c});return d}
...
function extend_search(a,c){searchLastIndex=a;search_outdated=false;window._gaq&&_gaq.push(["_trackEvent","Search","Load Search Page",current_search]);var b=function(d){c&&c();console.log(d);var f="",e=d.data.searchResults.digests;if(opt.prefetch&&onLine()==true){var g=0;setTimeout(function(){var k=e[g++];if(k){var n=k.waveId.replace(/[\/!].+/,"!conv+root");callbacks[wave.robot.fetchWave(k.waveId,n)]=function(o){if(o)prefetched_waves[k.waveId]=o;return true};runQueue();setTimeout(arguments.callee,
500)}},1E3)}for(var i=0;i<e.length;i++){d=e[i];d.title=d.title.replace(/\</g,"&lt;").replace(/\>/g,"&gt;");d.snippet=d.snippet.replace(/\</g,"&lt;").replace(/\>/g,"&gt;");unread_blips[d.waveId]=d.unreadCount;if(opt.old_results)f+='<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'><div class="search"><b>'+d.title+"</b> <span style='color:gray'>"+d.snippet+"</div></a>";else{var h=d.unreadCount>0?"<span class='unread_msg'><span class='bubble'>"+d.unreadCount+"</span> of "+d.blipCount+
"</span>":"<span class='read_msg'>"+d.blipCount+" msgs</span>",j=format_time(d.lastModified),l=opt.offline?'<br><span class="cachestate">cache</span>':"";h='<div id="'+d.waveId+'" class="search '+resultClass(d.waveId)+'"><div class="date">'+j+"<br>"+h+l+'</div><span class="title_'+(d.unreadCount>0?"unread":"read")+'">'+d.title+"</span> <span class='snippet'>"+d.snippet+"</span> </div>";f+=d.waveId.indexOf("treq")!=-1?'<a class="searchlink">'+h+"</a>":'<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'>'+
h+"</a>"}}search_container.innerHTML+=f;search_container.innerHTML+=onLine()==false?'<div class="footer"><b>Sorry!</b> No further waves have been cached.</div>':e.length<42?e.length==0?current_search.indexOf("is:unread")!=-1?"<div class='footer'><b>Yay</b>, no unread items!</div>":"<div class='footer'>Your search query has <b>zero results</b>. Try broaden your search?</div>":"<div class='footer'><b>Hooray!</b> You've reached the end of the universe!</div>":'<div class="footer" onclick="auto_extend(this);"><b>Extend</b> Search (Page '+
(a/42+1)+")</div>"};if(onLine()==true)callbacks[wave.robot.search(current_search,a||0,42)]=b;else{open_db();if(!window.db)return console.log("no database");db.transaction(function(d){d.executeSql("SELECT * FROM inbox",[],function(f,e){for(var g=[],i=0;i<e.rows.length;i++)g.push(JSON.parse(e.rows.item(i).result));b({data:{searchResults:{digests:g}}})})})}}function ch(a){hashHandler(a.href.substr(a.href.indexOf("#")),true)}
...
return true}else return false;if(g.data.waveletData){window.msg=g;searchmode(1);if(!opt.multipane)search_container.style.display="none";current_page=1;wave_container.style.display="";update_scroll();document.getElementById("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var i=Object.keys(msg.data.blips);else{i=[];for(var h in msg.data.blips)i.push(h)}chronological_blips=i.sort(function(l,k){return msg.data.blips[k].lastModifiedTime-msg.data.blips[l].lastModifiedTime})}scrollto_position=
-1;if(!auto_reload)if(!opt.no_scrollhistory){scrollTo(0,0);if(!opt.no_autoscroll)if(unread_blips[a])scrollto_position=unread_blips[a]-1;if(d)try{scrollto_position=chronological_blips.indexOf(d)}catch(j){}}auto_reload=false;if(!opt.multipane)document.getElementById("suggest").style.display="none";wave_container.innerHTML="";if(opt.owner_utils&&msg.data.waveletData.participants[0]==username)wave_container.innerHTML+='<div><button onclick="set_user_mode()">Everyone Read Only</button><button onclick="set_user_mode(\'FULL\')">Everyone Full Access</button></div>';
i=document.createElement("div");i.className="wavelet";h=document.createElement("a");h.innerHTML=" Add";h.className="addparticipant";h.href="javascript:void(0)";h.onclick=function(){var l=prompt("Enter Participant ID to Add");window._gaq&&_gaq.push(["_trackEvent","Modify","Add participant"]);if(l){if(l.indexOf("@")==-1)l+="@googlewave.com";wave.wavelet.participant.add(l,a,c);loadWave(a);auto_reload=true;runQueue()}};i.appendChild(h);i.appendChild(userList(g.data.waveletData.participants));wave_container.appendChild(i);
current_wave=a=a.replace(/\s/,"");current_wavelet=c;i=document.createElement("div");wave_container.appendChild(i);if(document.getElementById("chronos").checked)chronological_blip_render(i);else opt.recursive_renderer||!msg.data.waveletData.rootThread?recursive_blip_render(msg.data.waveletData.rootBlipId,i):bootstrap_thread_render(i);i=document.createElement("div");g=g.data.waveletData.tags.join(", ");if(g.length==0)g="(No Tags)";i.innerHTML="<b>Tags:</b> ";i.innerHTML=' <a href="javascript:add_tag()" style="float:right">Add</a>';
i.appendChild(document.createTextNode(g));i.className="tags";wave_container.appendChild(i);g=document.createElement("div");g.innerHTML='<a href="https://wave.google.com/wave/#restored:wave:'+escape(escape(a))+'" target="_blank">Open this wave in the official wave client</a>';g.className="footer";wave_container.appendChild(g);if(opt.bigspace){g=document.createElement("div");g.style.height="250px";wave_container.appendChild(g)}}else alert("The server sent nothing")};if(onLine()==false){open_db();if(!window.db)return console.log("no database");
db.transaction(function(g){g.executeSql("SELECT * FROM inbox WHERE waveid = ?",[a],function(i,h){var j=JSON.parse(h.rows.item(0).data);e(j)})})}else if(opt.prefetch&&prefetched_waves[a])e(JSON.parse(JSON.stringify(prefetched_waves[a])));else callbacks[wave.robot.fetchWave(a,c)]=e}if(!this.JSON)this.JSON={};
(function(){function a(j){return j<10?"0"+j:j}function c(j){f.lastIndex=0;return f.test(j)?'"'+j.replace(f,function(l){var k=i[l];return typeof k==="string"?k:"\\u"+("0000"+l.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+j+'"'}function b(j,l){var k,n,o,r,p=e,q,m=l[j];if(m&&typeof m==="object"&&typeof m.toJSON==="function")m=m.toJSON(j);if(typeof h==="function")m=h.call(l,j,m);switch(typeof m){case "string":return c(m);case "number":return isFinite(m)?String(m):"null";case "boolean":case "null":return String(m);
case "object":if(!m)return"null";e+=g;q=[];if(Object.prototype.toString.apply(m)==="[object Array]"){r=m.length;for(k=0;k<r;k+=1)q[k]=b(k,m)||"null";o=q.length===0?"[]":e?"[\n"+e+q.join(",\n"+e)+"\n"+p+"]":"["+q.join(",")+"]";e=p;return o}if(h&&typeof h==="object"){r=h.length;for(k=0;k<r;k+=1){n=h[k];if(typeof n==="string")if(o=b(n,m))q.push(c(n)+(e?": ":":")+o)}}else for(n in m)if(Object.hasOwnProperty.call(m,n))if(o=b(n,m))q.push(c(n)+(e?": ":":")+o);o=q.length===0?"{}":e?"{\n"+e+q.join(",\n"+e)+
"\n"+p+"}":"{"+q.join(",")+"}";e=p;return o}}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(){return isFinite(this.valueOf())?this.getUTCFullYear()+"-"+a(this.getUTCMonth()+1)+"-"+a(this.getUTCDate())+"T"+a(this.getUTCHours())+":"+a(this.getUTCMinutes())+":"+a(this.getUTCSeconds())+"Z":null};String.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(){return this.valueOf()}}var d=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,e,g,i={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},h;if(typeof JSON.stringify!=="function")JSON.stringify=function(j,l,k){var n;g=e="";if(typeof k==="number")for(n=0;n<k;n+=1)g+=" ";else if(typeof k==="string")g=k;if((h=l)&&typeof l!=="function"&&(typeof l!=="object"||typeof l.length!=="number"))throw Error("JSON.stringify");return b("",
{"":j})};if(typeof JSON.parse!=="function")JSON.parse=function(j,l){function k(o,r){var p,q,m=o[r];if(m&&typeof m==="object")for(p in m)if(Object.hasOwnProperty.call(m,p)){q=k(m,p);if(q!==undefined)m[p]=q;else delete m[p]}return l.call(o,r,m)}var n;j=String(j);d.lastIndex=0;if(d.test(j))j=j.replace(d,function(o){return"\\u"+("0000"+o.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(j.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){n=eval("("+j+")");return typeof l==="function"?k({"":n},""):n}throw new SyntaxError("JSON.parse");}})();function startup(){if(onLine()){wave.robot.notifyCapabilitiesHash();getUsername()}if(onLine()==false)document.body.className=document.body.className.replace("online","offline");location.hash.length<2?hashHandler("#search:in:inbox"):hashHandler(location.hash);for(var a=endQueue.length;a--;)endQueue[a]()}

...
function loadWave(a,c){var b=loading(a);if(onLine()==false)return window.offline_loadWave(a);var d=null,g;if(g=a.match(/(\w+\.\w+)\/(w\+\w+)\/\~\/(\w+\+\w+)\/(b\+\w+)/)){a=g[1]+"!"+g[2];c=g[1]+"!"+g[3];d=g[4]}else{a=a.replace("/","!");c=c||a.replace(/[\/!].+/,"!conv+root")}g=function(e){loading(b);console.log(e);window.msg=e;if(msg.error)if(msg.error.message.indexOf("not a participant")!=-1){alert("You are not a participant of the wave/wavelet. ");return true}else return false;if(e.data.waveletData){searchmode(1);
if(!opt.multipane)search_container.style.display="none";current_page=1;wave_container.style.display="";window.onscroll();document.getElementById("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var f=Object.keys(msg.data.blips);else{f=[];for(var h in msg.data.blips)f.push(h)}chronological_blips=f.sort(function(j,k){return msg.data.blips[k].lastModifiedTime-msg.data.blips[j].lastModifiedTime})}scrollto_position=-1;if(!auto_reload)if(!opt.no_scrollhistory){scrollTo(0,0);if(!opt.no_autoscroll)if(unread_blips[a])scrollto_position=
unread_blips[a];if(d)try{scrollto_position=chronological_blips.indexOf(d)}catch(i){}}auto_reload=false;if(!opt.multipane)document.getElementById("suggest").style.display="none";wave_container.innerHTML="";if(opt.owner_utils&&msg.data.waveletData.participants[0]==username)wave_container.innerHTML+='<div><button onclick="set_user_mode()">Everyone Read Only</button><button onclick="set_user_mode(\'FULL\')">Everyone Full Access</button></div>';f=document.createElement("div");f.className="wavelet";h=document.createElement("a");
h.innerHTML=" Add";h.className="addparticipant";h.style["float"]="right";h.href="javascript:void(0)";h.onclick=function(){var j=prompt("Enter Participant ID to Add");if(j){if(j.indexOf("@")==-1)j+="@googlewave.com";wave.wavelet.participant.add(j,a,c);loadWave(a);auto_reload=true;runQueue()}};f.appendChild(h);f.appendChild(userList(e.data.waveletData.participants));wave_container.appendChild(f);current_wave=a=a.replace(/\s/,"");current_wavelet=c;f=document.createElement("div");wave_container.appendChild(f);
if(document.getElementById("chronos").checked)chronological_blip_render(f);else opt.recursive_renderer||!msg.data.waveletData.rootThread?recursive_blip_render(msg.data.waveletData.rootBlipId,f):bootstrap_thread_render(f);f=document.createElement("div");e=e.data.waveletData.tags.join(", ");if(e.length==0)e="(None)";f.innerHTML="<b>Tags:</b> ";f.appendChild(document.createTextNode(e));f.innerHTML+=' <a href="javascript:add_tag()" style="float:right">Add</a>';f.className="tags";wave_container.appendChild(f);
e=document.createElement("div");e.innerHTML='<a href="https://wave.google.com/wave/#restored:wave:'+escape(escape(a))+'" target="_blank">Open this wave in the official wave client</a>';e.className="footer";wave_container.appendChild(e);wave_container.appendChild(document.createElement("br"))}else alert("The server sent nothing")};if(opt.prefetch&&prefetched_waves[a])g(JSON.parse(JSON.stringify(prefetched_waves[a])));else callbacks[wave.robot.fetchWave(a,c)]=g}if(!this.JSON)this.JSON={};

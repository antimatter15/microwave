...
opt.x.owner_utils="Enable utilities for wave creators";opt.x.no_autoscroll="Disable smart autoscroll to latest blip";opt.gadgets===undefined&&screen_size>900&&opt.fn.set("gadgets",true);if(opt.multipane===undefined&&screen_size>900){opt.fn.set("multipane",true);mobilewebkit&&opt.touchscroll===undefined&&opt.fn.set("touchscroll",true)}
...
function reset_touchscroll(){document.getElementById("wave_container_parent").style.width=innerWidth-250+"px";document.getElementById("wave_container").style.width=innerWidth-250+"px"}if(opt.touchscroll&&opt.multipane){addTouchScroll("wave_container_parent","search_parent_container");document.getElementById("wave_container_parent").style.overflow="hidden";reset_touchscroll();window.onorientationchange=reset_touchscroll;window.addEventListener("resize",reset_touchscroll,true)}
...
reply_text.focus();c()},Delete:function(){confirm("Are you sure you want to delete the blip?")&&setTimeout(function(){wave.blip["delete"](current_blip.blipId,current_blip.waveId,current_blip.waveletId);loadWave(current_blip.waveId);auto_reload=true;runQueue();c()},100);c()},Edit:function(){current_blip.dom.parentNode.insertBefore(create_edit_box(),current_blip.dom.nextSibling);var h=0;try{for(var i=current_blip.annotations.length,j=0;j<i&&current_blip.annotations[j].name!="conv/title";j++);if(j<i)h=
...
function animated_scroll(a,c){var b=a==window,d=b?pageYOffset:a.scrollTop;if(d!=c){var f=Math.min(1E3,2*Math.sqrt(42*Math.abs(c-d))),e,g=+new Date+f;(e=function(){var h=Math.min(1,1-(g-new Date)/f),i=(c-d)*h+d;if(b)scrollTo(0,i);else a.scrollTop=i;h<1&&setTimeout(e,0)})()}}
function scroll_wavepanel(a){if(opt.multipane)if(opt.touchscroll)touchscroll0.scrollTo(0,a);else if(opt.no_animate)document.getElementById("wave_container_parent").scrollTop=a;else animated_scroll(document.getElementById("wave_container_parent"),a);else opt.no_animate?scrollTo(0,a):animated_scroll(window,a)}
function scroll_searchpanel(a){if(opt.multipane)if(opt.touchscroll)touchscroll1.scrollTo(0,a);else if(opt.no_animate)document.getElementById("search_parent_container").scrollTop=a;else animated_scroll(document.getElementById("search_parent_container"),a);else opt.no_animate?scrollTo(0,a):animated_scroll(window,a)}

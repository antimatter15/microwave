...
a.style.top=0;a.style.left=0;a.style.width="100%";a.style.backgroundColor="#fff";a.style.padding="0";a.style.zIndex=99999999;a.onclick=function(d){d=d||window.event;(d.target||d.srcElement).tagName.toLowerCase()=="div"&&window.opt.fn.close()};document.body.appendChild(a)},handleBoolean:function(a){opt.fn.set(a.name,a.checked)},set:function(a,b){if(typeof opt.c[a]=="function")b=opt.c[a](b)||b;opt[a]=b;if(window.localStorage)localStorage["opt_"+a]=b.toString()}};opt.fn.init();
function searchStyle(a){if(cacheState[a]==2)return"fresh_cache";else if(cacheState[a]==1)return"old_cache";return""}
function onLine(){var a;a=opt.force_offline?false:navigator.onLine===undefined?true:navigator.onLine;if(a==false){var b=(+new Date-parseInt(localStorage.getItem("cache_last_updated")))/1E3;if(!isNaN(b)){var c="";c=b<90?"a minute":b<3600?Math.ceil(b/60)+" minutes":b<86400?Math.ceil(b/60/60)+" hours":Math.ceil(b/60/60/24)+" days";document.getElementById("offline_status").value="Offline (Cache "+c+" old)"}}return a}var cachequeue=[],db=null;
function open_db(){if(!window.db&&window.openDatabase)window.db=openDatabase("waves","1.0","Offline Wave Cache",1048576)}function offline_cache(){if(onLine()!=false){open_db();if(window.db){db.transaction(function(a){a.executeSql("DROP TABLE inbox")});localStorage.setItem("cache_last_updated",+new Date);callbacks[wave.robot.search("in:inbox",0,42)]=function(a){for(var b=a.data.searchResults.digests,c=0;c<b.length;c++){a=b[c];cachequeue.push(a)}setTimeout(cache_cycle,1E3)};runQueue()}}}
var cacheState={};try{if(window.localStorage&&localStorage.cacheState)for(var cs=JSON.parse(localStorage.getItem("cacheState")),i=0;i<cs.length;i++)cacheState[cs[i]]=1}catch(err$$1){}
function cache_cycle(){if(window.db){var a=null;if(a=cachequeue.shift()){callbacks[wave.robot.fetchWave(a.waveId,a.waveId.replace(/w\+.+/g,"conv+root"))]=function(b){db.transaction(function(c){c.executeSql("CREATE TABLE IF NOT EXISTS inbox (waveid, result, data, date)");c.executeSql("DELETE FROM inbox WHERE waveid = ?",[a.waveId],function(d){d.executeSql("INSERT INTO inbox (waveid, result, data, date) VALUES (?, ?, ?, ?)",[a.waveId,JSON.stringify(a),JSON.stringify(b),new Date-0],function(){if(document.getElementById(a.waveId))document.getElementById(a.waveId).className=
"search fresh_cache";cacheState[a.waveId]=2;var g=[];for(var f in cacheState)g.push(f);localStorage.setItem("cacheState",JSON.stringify(g));setTimeout(cache_cycle,1E3)})})});return true};runQueue()}}}function resultClass(a){if(cacheState[a])if(cacheState[a]==1)return"old_cache";else{if(cacheState[a]==2)return"fresh_cache"}else return""}window.console||(console={log:function(){}});var screen_size=document.documentElement.clientWidth||innerWidth,small_screen=screen_size<500,loadIds={};
searchLastIndex=0;current_search="";var edit_box,edit_text,search_container=document.getElementById("search_container"),wave_container=document.getElementById("wave_container"),mobilewebkit=navigator.userAgent.indexOf("WebKit")!=-1&&navigator.userAgent.indexOf("Mobile")!=-1,current_wave="",current_wavelet="",auto_reload=false,lasthash="chunkybacon",current_page=0,search_outdated=false,searchscroll=0,scrollto_position=-1;if(mobilewebkit)document.body.className+=" mobilewebkit";
if(!window.onLine)window.onLine=function(){return true};opt.appName="&mu;wave";opt.x.multipane="Enable multipane viewing experience (note, you must reload the page for changes to take effect)";opt.x.touchscroll="Add the TouchScroll library to do cool scrolly things on iPad Multipane (do not use on Desktop)";opt.x.no_animate="Disable animated scrolling effect";opt.x.no_scrollhistory="Do not save search scroll position and restore to it";opt.x.old_results="Old results panel style";opt.x.largeFont="Use a larger font";
opt.x.prefetch="Prefetch waves and load them, way faster and also not real time";opt.x.gadgets="Enable real wave gadget support (slow on mobile)";opt.x.render_state="If a gadget can not be internally rendered, display the gadget state";opt.x.bigspace="Add a large blank space under waves so the keyboard isn't messed up";opt.x.recursive_renderer="Use old version of tree wave renderer, only works on Wave Protocol 0.21 or below";opt.x.no_sig="Do not automatically add <i>posted with micro-wave</i> signature";
...
opt.swipe&&function(){var a=0,b=0,c=0,d=0,g,f=0,e=function(k){if(k.touches.length==1){c=k.touches[0].pageX;d=k.touches[0].pageY;g=k.touches[0].target;f=+new Date}},h=function(k){if(k.touches.length==1){a=k.touches[0].pageX;b=k.touches[0].pageY}},j=function(){var k=a-c,m=b-d,l=g;if(l){for(;!l.blipId&&l!=document.body;)l=l.parentNode;if(l.blipId)if(Math.abs(m)<10)if(Math.abs(k)>30)k>0?blip_prev(l.blipId):blip_next(l.blipId)}};if(mobilewebkit){document.body.addEventListener("touchstart",e,true);document.body.addEventListener("touchmove",
h,true);document.body.addEventListener("touchend",j,true)}else{document.body.addEventListener("mousedown",function(k){e({touches:[k]})},true);document.body.addEventListener("mousemove",function(k){h({touches:[k]})},true);document.body.addEventListener("mouseup",function(k){j({touches:[k]})},true)}}();
...
reply_text.focus();b()},Indented:function(){current_blip.dom.parentNode.insertBefore(create_reply_box(true),current_blip.dom.nextSibling);context_box.className="thread";context_box.style.display="";reply_text.focus();b()},Delete:function(){confirm("Are you sure you want to delete the blip?")&&setTimeout(function(){wave.blip["delete"](current_blip.blipId,current_blip.waveId,current_blip.waveletId);loadWave(current_blip.waveId);auto_reload=true;runQueue();b()},100);b()},Edit:function(){current_blip.dom.parentNode.insertBefore(create_edit_box(),
current_blip.dom.nextSibling);var h=0;try{for(var j=current_blip.annotations.length,k=0;k<j&&current_blip.annotations[k].name!="conv/title";k++);if(k<j)h=current_blip.annotations[k].range.end}catch(m){}edit_text.value=current_blip.content.substr(h+1);edit_text.focus();b()}};if(a.blipId==msg.data.waveletData.rootBlipId)g["Change Title"]=function(){var h=prompt("Enter new wavelet title",msg.data.waveletData.title);if(h){wave.wavelet.setTitle(h,a.waveId,a.waveletId);loadWave(a.waveId,a.waveletId);auto_reload=
...
function load_native_gadget(a,b,c,d){var g="gadget_frame"+Math.random().toString(36).substr(4);console.log(b);for(var f={myId:username,authorId:b.properties.author,participants:{}},e=msg.data.waveletData.participants,h=e.length;h--;)f.participants[e[h]]={id:e[h],displayName:e[h].replace(/@.*$/,""),thumbnailUrl:"https://wave.google.com/a/google.com/static/images/unknown.jpg"};gstates[g]={state:a,participants:f,blipId:c.blipId};var j=opt.gsa?"http://anti15.chemicalservers.com/debugwave.xml":opt.gsa1?
"http://anti15.chemicalservers.com/state.xml":b.properties.url,k="http://www.gmodules.com/gadgets/ifr?container=wave&view=default&debug=0&lang=en&country=ALL&nocache=0&wave=1&mid="+encodeURIComponent(g)+"&parent="+encodeURIComponent(location.protocol+"//"+location.host+location.pathname)+"&url="+encodeURIComponent(j);init_gadget_handler(function(){create_gadget_frame(g,k,d);console.log("creating "+g+" for gadget "+j)})}function native_gadget(){}
function renderGadget(a,b){var c={},d=[];for(var g in a.properties){if(g!="url"&&g!="author")c[g]=a.properties[g];d.push(g)}var f=document.createElement("div");f.style.margin="10px";var e=a.properties.url;f.innerHTML="<b>gadget</b> "+e+" <br>";if(window.opt.gadgets){load_native_gadget(c,a,b,f);return f}if(e=="http://wave-api.appspot.com/public/gadgets/areyouin/gadget.xml"){var h={y:[],n:[],m:[]};for(g in c)/:answer$/.test(g)&&h[c[g]].push(g.substr(0,g.length-7));for(var j in h){f.innerHTML+="<br><span style='color:red;font-weight:bold'>"+
{m:"Maybe",y:"Yes",n:"No"}[j]+"</span><br> ";if(h[j].length==0)f.innerHTML+="(None) <br>";for(g=0;g<h[j].length;g++)f.innerHTML+=h[j][g].replace(/@googlewave.com/g,"")+' <span style="color: gray;font-style: italic">'+(c[h[j][g]+":status"]||"")+"</span><br>"}}else if(e=="http://plus-one.appspot.com/plus-one.xml"){h=0;for(g in c)h+=parseInt(c[g]);f.innerHTML+="<br><b>Votes:</b> "+h+"/"+d.length}else if(e=="http://www.elizabethsgadgets.appspot.com/public/gadget.xml")f.innerHTML+="<br> <b>Pluses</b> ("+
(c.pluses||0)+")&nbsp;&nbsp;&nbsp;&nbsp;<b>Minuses</b> ("+(c.minuses||0)+")";else if(e=="http://pushyrobot.appspot.com/gadgets/github.xml")f.innerHTML+="<pre>"+JSON.stringify(JSON.parse(unescape(c.commit)),null,2)+"</pre>";else if(e=="http://everybodywave.appspot.com/gadget/image/gadget.xml")f.innerHTML+='<img src="'+c.imgUrl+'" width="'+c.imgWidth+'" height="'+c.imgHeight+'">';else if(e=="http://wavepollo.appspot.com/wavepollo/com.appspot.wavepollo.client.PolloWaveGadget.gadget.xml"){j={};for(h in c)if(h.indexOf("MVOTE_")==
0)if(g=h.match(/MVOTE_(.+)(OPT_.+)$/)){j[g[2]]||(j[g[2]]=[]);j[g[2]].push(g[1])}for(h in j){f.innerHTML+="<br><span style='color:red;font-weight:bold'>"+c[h]+"</span> ("+j[h].length+")<br> ";for(g=0;g<j[h].length;g++)f.innerHTML+=j[h][g].replace(/@googlewave.com/g,"")+", "}}else if(e=="https://statusee.appspot.com/gadget/statusee.xml")f.innerHTML+="<b>Status</b> "+({notstarted:"Not started",describing:"Describing",brainstorming:"Brainstorming",inprogress:"In Progress",inreview:"In Review",pending:"Pending",
testing:"Testing",completed:"Completed",rejected:"Rejected",canceled:"Canceled"}[c.sel]||c.sel.substr(7));else if(e=="http://wave-poll.googlecode.com/svn/trunk/src/poll.xml")for(h in c){j=JSON.parse(c[h]).participants;f.innerHTML+="<br><span style='color:red;font-weight:bold'>"+h.substr(7)+"</span> ("+j.length+")<br> ";for(g=0;g<j.length;g++)f.innerHTML+=j[g].replace(/@googlewave.com/g,"")+", "}else if(e=="https://everybodywave.appspot.com/gadget/miniroster/main.xml"){f.innerHTML+="<br><span style='color:red;font-weight:bold'>Assigned</span> ("+
d.length+")<br> ";for(h in c)f.innerHTML+=h.split("~")[3]+", "}else if(e=="http://www.nebweb.com.au/wave/likey.xml")f.innerHTML+="<br> <b>Like</b> ("+(c.likeCount||0)+")&nbsp;&nbsp;&nbsp;&nbsp;<b>Dislike</b> ("+(c.dislikeCount||0)+")";else if(window.opt.render_state||JSON.stringify(c).length<1337){console.log("Unknown Gadget",e);a=document.createTextNode(JSON.stringify(c,null,2));c=document.createElement("div");c.appendChild(a);f.innerHTML+='<div class="monospace">'+c.innerHTML+"</div>"}return f}
...
function animated_scroll(a,b){var c=a==window,d=c?pageYOffset:a.scrollTop;if(d!=b){var g=Math.min(1E3,2*Math.sqrt(42*Math.abs(b-d))),f,e=+new Date+g;(f=function(){var h=Math.min(1,1-(e-new Date)/g),j=(b-d)*(-Math.cos(h*Math.PI)/2+0.5)+d;if(c)scrollTo(0,j);else a.scrollTop=j;h<1&&setTimeout(f,0)})()}}
...
function markWaveRead(){var a=loading("mark as read");callbacks[wave.robot.folderAction("markAsRead",current_wave,current_wavelet)]=function(){loading(a)};hide_float();search_outdated=true;runQueue()}function archiveWave(){var a=loading("archive wave");callbacks[wave.robot.folderAction("archive",current_wave,current_wavelet)]=function(){loading(a)};hide_float();runQueue()}
...
modifyAction:a,range:{start:b,end:c}})}},blip:{"delete":function(a,b,c){return queueOp("wave.blip.delete",{waveletId:c,waveId:b,blipId:a})},replace:function(a,b,c,d){a=clean_text(a);return wave.document.modify({modifyHow:"REPLACE",values:["\n"+a]},b,c,d)},replace_range:function(a,b,c,d,g,f){a=clean_text(a);return wave.document.modify_range({modifyHow:"REPLACE",values:[a]},b,c,d,g,f)},update_element:function(a,b,c,d){return queueOp("wave.document.modify",{waveletId:d,waveId:c,blipId:b,modifyAction:{modifyHow:"UPDATE_ELEMENT",
elements:[{type:"GADGET",properties:a}]},modifyQuery:{restrictions:{},maxRes:1,elementMatch:"GADGET"}})},insert:function(a,b,c,d){a=clean_text(a);return wave.document.modify({modifyHow:"INSERT",values:["\n"+a]},b,c,d)},append:function(a,b,c,d){a=clean_text(a);return wave.document.modify({modifyHow:"INSERT_AFTER",values:[a]},b,c,d)},createChild:function(a,b,c,d){return queueOp("wave.blip.createChild",{waveletId:c,waveId:b,blipId:a,blipData:{waveletId:c,blipId:d,waveId:b,content:"",parentBlipId:a}})},
continueThread:function(a,b,c,d){return queueOp("wave.blip.continueThread",{waveletId:c,waveId:b,blipId:a,blipData:{waveletId:c,blipId:d,waveId:b,content:"",parentBlipId:a}})},contentCreateChild:function(a,b,c,d){var g="TBD_"+d+"_0x"+(Math.random()*9E5).toString(16);wave.blip.createChild(b,c,d,g);wave.blip.replace(a,g,c,d)},contentContinueThread:function(a,b,c,d){var g="TBD_"+d+"_0x"+(Math.random()*9E5).toString(16);wave.blip.continueThread(b,c,d,g);wave.blip.replace(a,g,c,d)}}};opt.appName="&mu;wave";
...
opt.swipe&&function(){var a=0,b=0,c=0,d=0,g,f=0,e=function(k){if(k.touches.length==1){c=k.touches[0].pageX;d=k.touches[0].pageY;g=k.touches[0].target;f=+new Date}},h=function(k){if(k.touches.length==1){a=k.touches[0].pageX;b=k.touches[0].pageY}},j=function(){var k=a-c,m=b-d,l=g;if(l){for(;!l.blipId&&l!=document.body;)l=l.parentNode;if(l.blipId)if(Math.abs(m)<10)if(Math.abs(k)>30)k>0?blip_prev(l.blipId):blip_next(l.blipId)}};if(mobilewebkit){document.body.addEventListener("touchstart",e,true);document.body.addEventListener("touchmove",
h,true);document.body.addEventListener("touchend",j,true)}else{document.body.addEventListener("mousedown",function(k){e({touches:[k]})},true);document.body.addEventListener("mousemove",function(k){h({touches:[k]})},true);document.body.addEventListener("mouseup",function(k){j({touches:[k]})},true)}}();
function chronological_blip_render(a){var b=[];for(var c in msg.data.blips)b.push(msg.data.blips[c]);b=b.sort(function(g,f){return g.lastModifiedTime-f.lastModifiedTime});var d=b.length-1;(function(){for(var g=Math.max(0,d-10);d>=g;d--){var f=d,e=blip_render(b[f].blipId,a);if(msg.data.blips[b[f].parentBlipId]&&e){var h=document.createElement("blockquote");f=msg.data.blips[b[f].parentBlipId];var j=f.contributors.join(", ").replace(/@googlewave.com/g,"")+":"+f.content;h.innerHTML=j.substr(0,140)+(j.length>
140?"...":"");h.setAttribute("onclick","msg.data.blips['"+f.blipId+"'].dom.scrollIntoView()");e.insertBefore(h,e.getElementsByTagName("div")[0].nextSibling)}}g&&setTimeout(arguments.callee,0)})()}
...
function thread_render(a,b){var c=msg.data.blips[a];if(c)if(!c.dom){var d=blip_render(a,b);c=c.replyThreadIds;var g=c.length;if(g!=0){var f=document.createElement("div");f.className="thread";b.appendChild(f);for(t=0;t<g;t++)for(var e=msg.data.threads[c[t]].blipIds,h=e.length,j=0;j<h;j++)thread_render(e[j],f)}return d}}
...
function renderBlip(a){for(var b=a.content+" ",c={},d={},g={},f=0;f<a.annotations.length;f++){var e=a.annotations[f];if(e.name.indexOf("user/d/")==0){var h=e.value.split(","),j=e.name.substr(7);if(new Date-parseInt(h[1])<6E4)g[j]="rgb("+Math.floor(205-Math.random()*100).toString()+","+Math.floor(205-Math.random()*100).toString()+","+Math.floor(205-Math.random()*100).toString()+")"}if(c[e.range.start])c[e.range.start].push(f);else c[e.range.start]=[f];if(d[e.range.end])d[e.range.end].push(f);else d[e.range.end]=
[f]}h={};var k=document.createElement("div"),m=null,l=null,o="";for(f=0;f<b.length;f++){if(c[f]||d[f]||a.elements[f]){o&&l.appendChild(document.createTextNode(o));o="";if(a.elements[f]){j=a.elements[f];if(j.type=="INLINE_BLIP")k.appendChild(inline_blip_render(j.properties.id));else if(j.type=="IMAGE"){m=document.createElement("div");m.innerHTML="<b>"+(j.properties.attachmentId||"")+"</b> "+(j.properties.caption||"")+"<br>";j.properties.url&&renderImage(j.properties.url,m);k.appendChild(m)}else if(j.type==
"INSTALLER"){m=document.createElement("div");m.style.border="3px dotted orange";m.style.margin="10px";m.innerHTML="&lt;<b>Extension Installer</b> "+j.properties.manifest+"&gt;";k.appendChild(m)}else if(j.type=="ATTACHMENT"){m=document.createElement("div");m.style.margin="10px";m.innerHTML="<b>"+j.properties.mimeType+"</b> "+j.properties.caption+"<br>";if(j.properties.mimeType.indexOf("image/")==0)renderImage(j.properties.attachmentUrl,m);else m.innerHTML+="<a href='"+j.properties.attachmentUrl+"'>Download</a>";
k.appendChild(m)}else if(j.type=="GADGET")k.appendChild(renderGadget(j,a));else j.type!="LINE"&&console.log("unknown element type",j.type,j.properties);m=document.createElement(j.properties.lineType||"p");if(j.properties.indent)m.style.marginLeft=j.properties.indent*20+"px";if(j.properties.alignment)m.style.textAlign={l:"left",c:"center",r:"right"}[j.properties.alignment];j.properties.direction&&m.setAttribute("dir",{l:"ltr",r:"rtl"}[j.properties.alignment]);k.appendChild(m)}if(c[f])for(var p=c[f],
s=p.length;s--;){e=a.annotations[p[s]];h[e.name]||(h[e.name]=[]);h[e.name].push(e.value);if(e.name.indexOf("user/e/")==0){j=e.name.substr(7);if(g[j]){var q=document.createElement("span");q.className="cursor";q.innerHTML=e.value.replace(/@.+/,"");q.style.backgroundColor=g[j];l.appendChild(q)}}}if(d[f]){p=d[f];for(s=p.length;s--;){e=a.annotations[p[s]];h[e.name].shift();h[e.name].length==0&&delete h[e.name]}}l=h["link/auto"]||h["link/manual"]||h["link/wave"]?document.createElement("a"):document.createElement("span");
m.appendChild(l);for(e in h){j=h[e][0];if(e.indexOf("style/")==0)l.style[e.substr(6)]=j;else if(e=="conv/title")l.style.fontWeight="bold";else if(e!="spell")if(e!="lang")if(e=="link/manual"||e=="link/auto")if(/^waveid:\/\//.test(j)){l.href="#wave:"+j.substr(9);l.setAttribute("onclick","ch(this)")}else if(/wave:(.+?)(\,$)/.test(j)){l.href="#wave:"+j.match(/wave:(.+?)(\,$)/)[1];l.setAttribute("onclick","ch(this)")}else{l.href=j;l.target="_blank"}else if(e=="link/wave"){l.href="#wave:"+j;l.setAttribute("onclick",
"ch(this)")}else e.indexOf("user/e")!=0&&e.indexOf("user/d")!=0&&e.indexOf("user/r")!=0&&console.log("unrecognized annotation",e,j)}}if(b.charAt(f)!="\n")o+=b.charAt(f)}o&&l.appendChild(document.createTextNode(o));return k}var queue=[],callbacks={},id_count=0;function queueOp(a,b,c){var d=(id_count++).toString();if(c)callbacks[d]=c;queue.push({id:d,method:a,params:b});return d}
...
function runQueue(){if(queue.length==0)return false;for(var a=[],b=0;b<queue.length;b++)a.push(queue[b].id);doXHR(JSON.stringify(queue),function(c){if(c.status==200){var d;try{d=JSON.parse(c.responseText)}catch(g){if(c.responseText.indexOf("Error 401")!=-1){alert("Your login token has expired\n"+c.responseText);return location="/?force_auth=true"}for(var f=0;f<a.length;f++){var e=null,h=a[f];if(callbacks[h]){try{e=callbacks[h]()}catch(j){}delete callbacks[h]}if(!e){alert("There was a server error, please try again. A");
c.responseText&&alert(c.responseText)}}}if(d){console.log(d);for(f=0;f<d.length;f++){h=d[f].id;e=null;if(callbacks[h]){e=callbacks[h](d[f]);delete callbacks[h]}if(d[f].error&&!e){if(d[f].error.code==401){alert("Your login token has expired\n"+c.responseText);return location="/?force_auth=true"}alert("Error "+d[f].error.code+": "+d[f].error.message)}}}}else for(f=0;f<a.length;f++){e=null;h=a[f];if(callbacks[h]){try{e=callbacks[h]()}catch(k){}delete callbacks[h]}if(!e){alert("There was a server error, please try again. B");
...
function extend_search(a,b){searchLastIndex=a;search_outdated=false;var c=function(d){b&&b();console.log(d);var g="",f=d.data.searchResults.digests;if(opt.prefetch&&onLine()==true){var e=0;setTimeout(function(){var l=f[e++];if(l){var o=l.waveId.replace(/[\/!].+/,"!conv+root");callbacks[wave.robot.fetchWave(l.waveId,o)]=function(p){if(p)prefetched_waves[l.waveId]=p;return true};runQueue();setTimeout(arguments.callee,500)}},1E3)}for(var h=0;h<f.length;h++){d=f[h];d.title=d.title.replace(/\</g,"&lt;").replace(/\>/g,
"&gt;");d.snippet=d.snippet.replace(/\</g,"&lt;").replace(/\>/g,"&gt;");unread_blips[d.waveId]=d.unreadCount;if(opt.old_results)g+='<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'><div class="search"><b>'+d.title+"</b> <span style='color:gray'>"+d.snippet+"</div></a>";else{var j=d.unreadCount>0?"<span class='unread_msg'><span class='bubble'>"+d.unreadCount+"</span> of "+d.blipCount+"</span>":"<span class='read_msg'>"+d.blipCount+" msgs</span>",k=format_time(d.lastModified),m=
opt.offline?'<br><span class="cachestate">cache</span>':"";j='<div id="'+d.waveId+'" class="search '+resultClass(d.waveId)+'"><div class="date">'+k+"<br>"+j+m+'</div><span class="title_'+(d.unreadCount>0?"unread":"read")+'">'+d.title+"</span> <span class='snippet'>"+d.snippet+"</span> </div>";g+=d.waveId.indexOf("treq")!=-1?'<a class="searchlink">'+j+"</a>":'<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'>'+j+"</a>"}}search_container.innerHTML+=g;search_container.innerHTML+=onLine()==
false?'<div class="footer"><b>Sorry!</b> No further waves have been cached.</div>':f.length<42?f.length==0?current_search.indexOf("is:unread")!=-1?"<div class='footer'><b>Yay</b>, no unread items!</div>":"<div class='footer'>Your search query has <b>zero results</b>. Try broaden your search?</div>":"<div class='footer'><b>Hooray!</b> You've reached the end of the universe!</div>":'<div class="footer" onclick="auto_extend(this);"><b>Extend</b> Search (Page '+(a/42+1)+")</div>"};if(onLine()==true)callbacks[wave.robot.search(current_search,
a||0,42)]=c;else{open_db();if(!window.db)return console.log("no database");db.transaction(function(d){d.executeSql("SELECT * FROM inbox",[],function(g,f){for(var e=[],h=0;h<f.rows.length;h++)e.push(JSON.parse(f.rows.item(h).result));c({data:{searchResults:{digests:e}}})})})}}function ch(a){hashHandler(a.href.substr(a.href.indexOf("#")),true)}
...
if(e.data.waveletData){window.msg=e;searchmode(1);if(!opt.multipane)search_container.style.display="none";current_page=1;wave_container.style.display="";update_scroll();document.getElementById("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var h=Object.keys(msg.data.blips);else{h=[];for(var j in msg.data.blips)h.push(j)}chronological_blips=h.sort(function(m,l){return msg.data.blips[l].lastModifiedTime-msg.data.blips[m].lastModifiedTime})}scrollto_position=-1;if(!auto_reload)if(!opt.no_scrollhistory){scrollTo(0,
...
h=document.createElement("div");h.className="wavelet";j=document.createElement("a");j.innerHTML=" Add";j.className="addparticipant";j.href="javascript:void(0)";j.onclick=function(){var m=prompt("Enter Participant ID to Add");if(m){if(m.indexOf("@")==-1)m+="@googlewave.com";wave.wavelet.participant.add(m,a,b);loadWave(a);auto_reload=true;runQueue()}};h.appendChild(j);h.appendChild(userList(e.data.waveletData.participants));wave_container.appendChild(h);current_wave=a=a.replace(/\s/,"");current_wavelet=
b;h=document.createElement("div");wave_container.appendChild(h);if(document.getElementById("chronos").checked)chronological_blip_render(h);else opt.recursive_renderer||!msg.data.waveletData.rootThread?recursive_blip_render(msg.data.waveletData.rootBlipId,h):bootstrap_thread_render(h);h=document.createElement("div");e=e.data.waveletData.tags.join(", ");if(e.length==0)e="(No Tags)";h.innerHTML="<b>Tags:</b> ";h.innerHTML=' <a href="javascript:add_tag()" style="float:right">Add</a>';h.appendChild(document.createTextNode(e));
h.className="tags";wave_container.appendChild(h);e=document.createElement("div");e.innerHTML='<a href="https://wave.google.com/wave/#restored:wave:'+escape(escape(a))+'" target="_blank">Open this wave in the official wave client</a>';e.className="footer";wave_container.appendChild(e);if(opt.bigspace){e=document.createElement("div");e.style.height="250px";wave_container.appendChild(e)}}else alert("The server sent nothing")};if(onLine()==false){open_db();if(!window.db)return console.log("no database");
db.transaction(function(e){e.executeSql("SELECT * FROM inbox WHERE waveid = ?",[a],function(h,j){var k=JSON.parse(j.rows.item(0).data);f(k)})})}else if(opt.prefetch&&prefetched_waves[a])f(JSON.parse(JSON.stringify(prefetched_waves[a])));else callbacks[wave.robot.fetchWave(a,b)]=f}if(!this.JSON)this.JSON={};
(function(){function a(k){return k<10?"0"+k:k}function b(k){g.lastIndex=0;return g.test(k)?'"'+k.replace(g,function(m){var l=h[m];return typeof l==="string"?l:"\\u"+("0000"+m.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+k+'"'}function c(k,m){var l,o,p,s,q=f,r,n=m[k];if(n&&typeof n==="object"&&typeof n.toJSON==="function")n=n.toJSON(k);if(typeof j==="function")n=j.call(m,k,n);switch(typeof n){case "string":return b(n);case "number":return isFinite(n)?String(n):"null";case "boolean":case "null":return String(n);
case "object":if(!n)return"null";f+=e;r=[];if(Object.prototype.toString.apply(n)==="[object Array]"){s=n.length;for(l=0;l<s;l+=1)r[l]=c(l,n)||"null";p=r.length===0?"[]":f?"[\n"+f+r.join(",\n"+f)+"\n"+q+"]":"["+r.join(",")+"]";f=q;return p}if(j&&typeof j==="object"){s=j.length;for(l=0;l<s;l+=1){o=j[l];if(typeof o==="string")if(p=c(o,n))r.push(b(o)+(f?": ":":")+p)}}else for(o in n)if(Object.hasOwnProperty.call(n,o))if(p=c(o,n))r.push(b(o)+(f?": ":":")+p);p=r.length===0?"{}":f?"{\n"+f+r.join(",\n"+f)+
...
g=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,f,e,h={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},j;if(typeof JSON.stringify!=="function")JSON.stringify=function(k,m,l){var o;e=f="";if(typeof l==="number")for(o=0;o<l;o+=1)e+=" ";else if(typeof l==="string")e=l;if((j=m)&&typeof m!=="function"&&(typeof m!=="object"||typeof m.length!=="number"))throw Error("JSON.stringify");return c("",

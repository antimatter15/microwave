...
if(opt.multipane===undefined&&screen_size>900){opt.fn.set("multipane",true);mobilewebkit&&opt.touchscroll===undefined&&opt.fn.set("touchscroll",true)}
...
current_blip.dom.nextSibling);var h=0;try{for(var i=current_blip.annotations.length,j=0;j<i&&current_blip.annotations[j].name!="conv/title";j++);if(j<i)h=current_blip.annotations[j].range.end}catch(l){}edit_text.value=current_blip.content.substr(h+1);edit_text.focus();c()}};if(a.blipId==msg.data.waveletData.rootBlipId)f["Change Title"]=function(){var h=prompt("Enter new wavelet title",msg.data.waveletData.title);if(h){wave.wavelet.setTitle(h,a.waveId,a.waveletId);loadWave(a.waveId,a.waveletId);auto_reload=
true;runQueue()}};for(var e in f){var g=document.createElement("a");g.href="javascript:void(0)";g.onclick=f[e];g.innerHTML=e;d.appendChild(g);d.appendChild(document.createTextNode(" / "))}g=document.createElement("a");g.href="javascript:void(0);";g.style.color="red";g.innerHTML="Close";g.onclick=function(){c()};d.appendChild(g);d.className="contextmenu";return ctx_menu=d}
...
opt.x.swipe="Enable swiping gesture to navigate between blips (swipe left = prev, swipe right = next)";opt.x.keyboard="Enable keyboard shortcuts";opt.gadgets===undefined&&screen_size>900&&opt.fn.set("gadgets",true);opt.bigspace===undefined&&mobilewebkit&&opt.fn.set("bigspace",true);if(opt.multipane===undefined&&screen_size>900){opt.fn.set("multipane",true);mobilewebkit&&opt.touchscroll===undefined&&opt.fn.set("touchscroll",true)}
if(opt.multipane){document.getElementById("search_parent").insertBefore(document.getElementById("appheader"),document.getElementById("search_parent").firstChild);document.body.className+=" multipane";document.getElementById("header").innerHTML="&mu;wave";wave_container.innerHTML="<div style='padding:40px'>No waves loaded yet</div>";location.hash.indexOf("search:")==-1&&setTimeout(function(){autosearch("in:inbox");runQueue()},500)}
...
function thread_render(a,c){var b=msg.data.blips[a];if(b)if(!b.dom){var d=blip_render(a,c);b=b.replyThreadIds;var f=b.length;if(f!=0){var e=document.createElement("div");e.className="thread";c.appendChild(e);for(t=0;t<f;t++)for(var g=msg.data.threads[b[t]].blipIds,h=g.length,i=0;i<h;i++)thread_render(g[i],e)}return d}}
...
j.appendChild(l)}else if(i.type=="GADGET")j.appendChild(renderGadget(i,a));else i.type!="LINE"&&console.log("unknown element type",i.type,i.properties);l=document.createElement(i.properties.lineType||"p");if(i.properties.indent)l.style.marginLeft=i.properties.indent*20+"px";if(i.properties.alignment)l.style.textAlign={l:"left",c:"center",r:"right"}[i.properties.alignment];i.properties.direction&&l.setAttribute("dir",{l:"ltr",r:"rtl"}[i.properties.alignment]);j.appendChild(l)}if(b[e])for(var o=b[e],
r=o.length;r--;){g=a.annotations[o[r]];h[g.name]||(h[g.name]=[]);h[g.name].push(g.value);if(g.name.indexOf("user/e/")==0){i=g.name.substr(7);if(f[i]){var p=document.createElement("span");p.className="cursor";p.innerHTML=g.value.replace(/@.+/,"");p.style.backgroundColor=f[i];k.appendChild(p)}}}if(d[e]){o=d[e];for(r=o.length;r--;){g=a.annotations[o[r]];h[g.name].shift();h[g.name].length==0&&delete h[g.name]}}k=h["link/auto"]||h["link/manual"]||h["link/wave"]?document.createElement("a"):document.createElement("span");
...
function extend_search(a,c){searchLastIndex=a;search_outdated=false;callbacks[wave.robot.search(current_search,a||0,42)]=function(b){c&&c();h=b;console.log(h);b="";var d,f=h.data.searchResults.digests;if(opt.prefetch){var e=0;setTimeout(function(){var j=f[e++];if(j){var l=j.waveId.replace(/[\/!].+/,"!conv+root");callbacks[wave.robot.fetchWave(j.waveId,l)]=function(k){if(k)prefetched_waves[j.waveId]=k;return true};runQueue();setTimeout(arguments.callee,500)}},1E3)}for(var g=0;g<f.length;g++){d=f[g];
...
function loadWave(a,c){var b=loading(a);if(onLine()==false)return window.offline_loadWave(a);var d=null,f;if(f=a.match(/(\w+\.\w+)\/(w\+\w+)\/\~\/(\w+\+\w+)\/(b\+\w+)/)){a=f[1]+"!"+f[2];c=f[1]+"!"+f[3];d=f[4]}else{a=a.replace("/","!");c=c||a.replace(/[\/!].+/,"!conv+root")}f=function(e){loading(b);console.log(e);if(e.error)if(e.error.message.indexOf("not a participant")!=-1){alert("You are not a participant of the wave/wavelet. \nThis may be due to a bug in the current version of the data api which does not allow access to waves unless you are explicitly a participant. don't blame me");
return true}else return false;if(e.data.waveletData){window.msg=e;searchmode(1);if(!opt.multipane)search_container.style.display="none";current_page=1;wave_container.style.display="";update_scroll();document.getElementById("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var g=Object.keys(msg.data.blips);else{g=[];for(var h in msg.data.blips)g.push(h)}chronological_blips=g.sort(function(j,l){return msg.data.blips[l].lastModifiedTime-msg.data.blips[j].lastModifiedTime})}scrollto_position=
...
g=document.createElement("div");g.className="wavelet";h=document.createElement("a");h.innerHTML=" Add";h.className="addparticipant";h.style["float"]="right";h.href="javascript:void(0)";h.onclick=function(){var j=prompt("Enter Participant ID to Add");if(j){if(j.indexOf("@")==-1)j+="@googlewave.com";wave.wavelet.participant.add(j,a,c);loadWave(a);auto_reload=true;runQueue()}};g.appendChild(h);g.appendChild(userList(e.data.waveletData.participants));wave_container.appendChild(g);current_wave=a=a.replace(/\s/,
"");current_wavelet=c;g=document.createElement("div");wave_container.appendChild(g);if(document.getElementById("chronos").checked)chronological_blip_render(g);else opt.recursive_renderer||!msg.data.waveletData.rootThread?recursive_blip_render(msg.data.waveletData.rootBlipId,g):bootstrap_thread_render(g);g=document.createElement("div");e=e.data.waveletData.tags.join(", ");if(e.length==0)e="(No Tags)";g.innerHTML="<b>Tags:</b> ";g.innerHTML=' <a href="javascript:add_tag()" style="float:right">Add</a>';
g.appendChild(document.createTextNode(e));g.className="tags";wave_container.appendChild(g);e=document.createElement("div");e.innerHTML='<a href="https://wave.google.com/wave/#restored:wave:'+escape(escape(a))+'" target="_blank">Open this wave in the official wave client</a>';e.className="footer";wave_container.appendChild(e);if(opt.bigspace){e=document.createElement("div");e.style.height="250px";wave_container.appendChild(e)}}else alert("The server sent nothing")};if(opt.prefetch&&prefetched_waves[a])f(JSON.parse(JSON.stringify(prefetched_waves[a])));
else callbacks[wave.robot.fetchWave(a,c)]=f}if(!this.JSON)this.JSON={};
(function(){function a(j){return j<10?"0"+j:j}function c(j){f.lastIndex=0;return f.test(j)?'"'+j.replace(f,function(l){var k=h[l];return typeof k==="string"?k:"\\u"+("0000"+l.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+j+'"'}function b(j,l){var k,n,o,r,p=e,q,m=l[j];if(m&&typeof m==="object"&&typeof m.toJSON==="function")m=m.toJSON(j);if(typeof i==="function")m=i.call(l,j,m);switch(typeof m){case "string":return c(m);case "number":return isFinite(m)?String(m):"null";case "boolean":case "null":return String(m);
case "object":if(!m)return"null";e+=g;q=[];if(Object.prototype.toString.apply(m)==="[object Array]"){r=m.length;for(k=0;k<r;k+=1)q[k]=b(k,m)||"null";o=q.length===0?"[]":e?"[\n"+e+q.join(",\n"+e)+"\n"+p+"]":"["+q.join(",")+"]";e=p;return o}if(i&&typeof i==="object"){r=i.length;for(k=0;k<r;k+=1){n=i[k];if(typeof n==="string")if(o=b(n,m))q.push(c(n)+(e?": ":":")+o)}}else for(n in m)if(Object.hasOwnProperty.call(m,n))if(o=b(n,m))q.push(c(n)+(e?": ":":")+o);o=q.length===0?"{}":e?"{\n"+e+q.join(",\n"+e)+
...
f=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,e,g,h={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},i;if(typeof JSON.stringify!=="function")JSON.stringify=function(j,l,k){var n;g=e="";if(typeof k==="number")for(n=0;n<k;n+=1)g+=" ";else if(typeof k==="string")g=k;if((i=l)&&typeof l!=="function"&&(typeof l!=="object"||typeof l.length!=="number"))throw Error("JSON.stringify");return b("",
...
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){n=eval("("+j+")");return typeof l==="function"?k({"":n},""):n}throw new SyntaxError("JSON.parse");}})();function startup(){wave.robot.notifyCapabilitiesHash();getUsername();location.hash.length<2?hashHandler("#search:in:inbox"):hashHandler(location.hash)}function auto_start(){window.NO_STARTUP||startup();window.offline_cache&&setTimeout(offline_cache,1337)}setTimeout(auto_start,0);

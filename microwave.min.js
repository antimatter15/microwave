...
opt.swipe&&function(){var a=0,c=0,b=0,d=0,f,e=0,g=function(j){if(j.touches.length==1){b=j.touches[0].pageX;d=j.touches[0].pageY;f=j.touches[0].target;e=+new Date}},i=function(j){if(j.touches.length==1){a=j.touches[0].pageX;c=j.touches[0].pageY}},h=function(){var j=+new Date-e,k=a-b,l=c-d,m=f;if(m){for(;!m.blipId&&m!=document.body;)m=m.parentNode;if(m.blipId)if(Math.abs(l)<30&&j<500)if(Math.abs(k)>60)k>0?blip_next(m.blipId):blip_prev(m.blipId)}};if(mobilewebkit){document.body.addEventListener("touchstart",
...
f.innerHTML="<div style='float:right;color:#555'>"+format_time(b.lastModifiedTime).toString()+e;f.appendChild(userList(b.contributors));d.ondrop=function(g){g.stopPropagation();g.preventDefault();g=g.dataTransfer.files;for(var i=0;i<g.length;i++){var h=g[i],j=new FileReader;j.onload=function(k){k=k.target.result;if(k.indexOf("data:image/png;base64,")==0){k=k.substr(22);wave.blip.upload_attachment(k,h.name,a,b.waveId,b.waveletId)}else alert("data URL is not base64!")};j.readAsDataURL(h)}loadWave(current_blip.waveId);
runQueue()};d.onclick=doNothing;f.onclick=doNothing;d.insertBefore(f,d.firstChild);c.appendChild(d);a==chronological_blips[scrollto_position]&&setTimeout(function(){blip_scroll(scrollto_position)},500);return d}}
...
[e]}i={};var j=document.createElement("div"),k=null,l=null,m="";for(e=0;e<c.length;e++){if(b[e]||d[e]||a.elements[e]){m&&l.appendChild(document.createTextNode(m));m="";if(a.elements[e]){h=a.elements[e];if(h.type=="INLINE_BLIP")j.appendChild(inline_blip_render(h.properties.id));else if(h.type=="IMAGE"){k=document.createElement("div");k.innerHTML="<b>"+(h.properties.attachmentId||"")+"</b> "+(h.properties.caption||"")+"<br>";h.properties.url&&renderImage(h.properties.url,k);j.appendChild(k)}else if(h.type==
"INSTALLER"){k=document.createElement("div");k.style.border="3px dotted orange";k.style.margin="10px";k.innerHTML="&lt;<b>Extension Installer</b> "+h.properties.manifest+"&gt;";j.appendChild(k)}else if(h.type=="ATTACHMENT"){k=document.createElement("div");k.style.margin="10px";k.innerHTML="<b>"+h.properties.mimeType+"</b> "+h.properties.caption+"<br>";if(h.properties.mimeType&&h.properties.mimeType.indexOf("image/")==0)renderImage(h.properties.attachmentUrl,k);else k.innerHTML+="<a href='"+h.properties.attachmentUrl+
"'>Download</a>";j.appendChild(k)}else if(h.type=="GADGET")j.appendChild(renderGadget(h,a));else h.type!="LINE"&&console.log("unknown element type",h.type,h.properties);k=document.createElement(h.properties.lineType||"p");if(h.properties.indent)k.style.marginLeft=h.properties.indent*20+"px";if(h.properties.alignment)k.style.textAlign={l:"left",c:"center",r:"right"}[h.properties.alignment];h.properties.direction&&k.setAttribute("dir",{l:"ltr",r:"rtl"}[h.properties.alignment]);j.appendChild(k)}if(b[e])for(var n=
b[e],o=n.length;o--;){g=a.annotations[n[o]];i[g.name]||(i[g.name]=[]);i[g.name].push(g.value);if(g.name.indexOf("user/e/")==0){h=g.name.substr(7);if(f[h]){var p=document.createElement("span");p.className="cursor";p.innerHTML=g.value.replace(/@.+/,"");p.style.backgroundColor=f[h];l.appendChild(p)}}}if(d[e]){n=d[e];for(o=n.length;o--;){g=a.annotations[n[o]];i[g.name].shift();i[g.name].length==0&&delete i[g.name]}}l=i["link/auto"]||i["link/manual"]||i["link/wave"]?document.createElement("a"):document.createElement("span");
k.appendChild(l);for(g in i){h=i[g][0];if(g.indexOf("style/")==0)l.style[g.substr(6)]=h;else if(g=="conv/title")l.style.fontWeight="bold";else if(g!="spell")if(g!="lang")if(g=="link/manual"||g=="link/auto")if(/^waveid:\/\//.test(h)){l.href="#wave:"+h.substr(9);l.setAttribute("onclick","ch(this)")}else if(/wave:(.+?)(\,$)/.test(h)){l.href="#wave:"+h.match(/wave:(.+?)(\,$)/)[1];l.setAttribute("onclick","ch(this)")}else{l.href=h;l.target="_blank"}else if(g=="link/wave"){l.href="#wave:"+h;l.setAttribute("onclick",
...
"</span>":"<span class='read_msg'>"+d.blipCount+" msgs</span>",j=format_time(d.lastModified),k=opt.offline?' <span class="cachestate">cache</span>':"";h='<div id="'+d.waveId+'" class="search '+resultClass(d.waveId)+'"><div class="date">'+j+"<br>"+h+k+'</div><span class="title_'+(d.unreadCount>0?"unread":"read")+'">'+d.title+"</span> <span class='snippet'>"+d.snippet+"</span> </div>";f+='<a href="#wave:'+d.waveId+'" class="searchlink" onclick=\'ch(this)\'>'+h+"</a>"}}search_container.innerHTML+=f;
...
return true}else return false;if(g.data.waveletData){window.msg=g;searchmode(1);if(!opt.multipane)search_container.style.display="none";current_page=1;wave_container.style.display="";update_scroll();getEl("floating_menu").style.display="";if(!opt.no_autoscroll){if(Object.keys)var h=Object.keys(msg.data.blips);else{h=[];for(var j in msg.data.blips)h.push(j)}chronological_blips=h.sort(function(l,m){return msg.data.blips[m].lastModifiedTime-msg.data.blips[l].lastModifiedTime})}scrollto_position=-1;if(!auto_reload)if(!opt.no_scrollhistory){scrollTo(0,
0);if(!opt.no_autoscroll)if(unread_blips[a])scrollto_position=unread_blips[a]-1;if(d)try{scrollto_position=chronological_blips.indexOf(d)}catch(k){}}auto_reload=false;if(!opt.multipane)getEl("suggest").style.display="none";wave_container.innerHTML="";if(opt.owner_utils&&msg.data.waveletData.participants[0]==username)wave_container.innerHTML+='<div><button onclick="set_user_mode()">Everyone Read Only</button><button onclick="set_user_mode(\'FULL\')">Everyone Full Access</button></div>';h=document.createElement("div");
h.className="wavelet";j=document.createElement("a");j.innerHTML=" Add";j.className="addparticipant";j.href="javascript:void(0)";j.onclick=function(){var l=prompt("Enter Participant ID to Add");window._gaq&&_gaq.push(["_trackEvent","Modify","Add participant"]);if(l){if(l.indexOf("@")==-1)l+="@googlewave.com";wave.wavelet.participant.add(l,a,c);loadWave(a);auto_reload=true;runQueue()}};h.appendChild(j);h.appendChild(userList(g.data.waveletData.participants));wave_container.appendChild(h);current_wave=
